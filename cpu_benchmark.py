...
def test1():
    start = default_timer()
    genome_sizes = range(int(250e3), int(2000e3 + 1), int(250e3))
    trials = 1 # trials per size
    w = 16 # window size
    bases = "ACGT"
    pssm = [{base: np.random.uniform(-7.5, 2.0) for base in bases} for _ in range(w)] # generate PSSM
    data = []
    for n in genome_sizes * trials:
        # Generate random genome
        seq = "".join([bases[b] for b in np.random.randint(0, 3, int(n))])
    ...
    print "\n\nBenchmarked %g bp in %.2f seconds." % (np.int64(sum(genome_sizes)) * trials, default_timer() - start)
